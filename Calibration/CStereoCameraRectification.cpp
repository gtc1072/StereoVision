	...
	Matrix_Type left_proj, Matrix_Type right_proj)
{
	...
	*pYmap_right = new Data_Type[imageWidth * imageHeight];
	...
	for (int j = 0; j < imageHeight; ++j)
	{
		...
		{
			point(0, 0) = i; point(1, 0) = j; point(2, 0) = 1;
			...
			{
				//tx = ty = INVALID_MAP;
				(*pXmap_left)[j * imageWidth + i] = INVALID_MAP;
				(*pYmap_left)[j * imageWidth + i] = INVALID_MAP;
			}
			...
			{
				//tx = ty = INVALID_MAP;
				(*pXmap_right)[j * imageWidth + i] = INVALID_MAP;
				(*pYmap_right)[j * imageWidth + i] = INVALID_MAP;
			}
		}
	}
	return ret;
